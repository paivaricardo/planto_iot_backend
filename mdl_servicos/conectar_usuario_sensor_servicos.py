...
def desconectar_usuario_sensor_servico(uuid_sensor_atuador: UUID, email_usuario: str):
    """
    Serviço para desconectar um usuário a um sensor ou atuador. O usuário será identificado pelo seu id, ao passo que o sensor ou atuador pelo seu uuid.

    Retorna um dicionário com códigos que representam o status da conexão com o sensor ou atuador, de acordo com o seguinte:
    1 - Desconexão realizada com sucesso.
    2 - Usuário já está desconectado do sensor ou atuador.
    3 - Erro. Sensor ou atuador não existe na base de dados.
    4 - Erro. Sensor/atuador existe, mas o usuário não possui autorização para acesso ao sensor/atuador.
    5 - Erro. Cadastro do sensor não foi completado. É necessário completar o cadastro do sensor para que ele possa ser utilizado.
    6 - Erro. Usuário não existe na base de dados.
    7 - Erro desconhecido ao tentar desconectar o usuário ao sensor/atuador.
    """
    try:
        # Chamar o DAO para verificar se o sensor ou atuador existe na base de dados, e se o cadastro foi completo ou não
        sensor_atuador_existe = dao_verificar_sensor_atuador.verificar_existencia_sensor_atuador_base_dados(
        ...
        # Se o sensor ou atuador não existe na base de dados, retornar o código correspondente (vide acima)
        if not sensor_atuador_existe["sensor_atuador_existe_bd"]:
            return {"cod_status_desconexao": 3, "mensagem": "Erro. Sensor ou atuador não existe na base de dados."}
        ...
        # Se o usuário não existe na base de dados, retornar o código correspondente (vide acima)
        if not id_usuario:
            return {"cod_status_desconexao": 6, "mensagem": "Erro. Usuário não existe na base de dados."}
        ...
        # Se não houver autorização para o usuário, retornar o código correspondente (vide acima)
        if len(autorizacoes_usuario) == 0:
            return {"cod_status_desconexao": 4, "mensagem": "Erro. Sensor/atuador existe, mas o usuário não possui autorização para acesso ao sensor/atuador."}

        # Se o cadastro do sensor não foi completado, retornar o código correspondente (vide acima)
        if not sensor_atuador_existe["sensor_atuador_foi_cadastrado"]:
            return {"cod_status_desconexao": 5, "mensagem": "Erro. Cadastro do sensor não foi completado. É necessário completar o cadastro do sensor para que ele possa ser utilizado."}
        ...
        if autorizacoes_usuario[0]["visualizacao_ativa"]:
            sensor_atuador_desconectado_usuario = dao_conectar_sensor_atuador_usuario.desconectar_sensor_atuador_usuario_dao(
                autorizacoes_usuario[0])

            # Se a desconexão for realizada com sucesso, retornar o código correspondente (vide acima)
            if sensor_atuador_desconectado_usuario:
                return {"cod_status_desconexao": 1, "mensagem": "Desconexão realizada com sucesso."}
    except Exception as e:
        return {"cod_status_desconexao": 7, "mensagem": f"Erro desconhecido ao tentar desconectar o usuário ao sensor/atuador: {str(e)}"}
